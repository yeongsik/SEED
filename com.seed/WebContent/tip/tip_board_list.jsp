...
<!-- 페이징 처리 -->
	<div style="margin:0 auto;">
		<c:if test="${listcount > 0 }">
			...
			<!-- 1페이지로 이동 -->	
			<a href="./TipListAction.tip?page=1" style="text-decoration: none;"> << </a>
				
			
			<!-- 이전 블럭으로 이동 -->
			<c:if test="${startPage > 10 }">
				<a href="./TipListAction.tip?page=${startPage - 10}">[이전]</a>
			</c:if>
			...
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				...
				<c:if test="${i != page }"> <!-- 현재페이지 아닐경우 링크 추가-->
					<a href="./TipListAction.tip?page=${i}">[${i}]</a>
				</c:if>
			</c:forEach>
			<!-- 다음 블럭으로 이동 -->
			<c:if test="${endPage < pageCount}">
				<a href="./TipListAction.tip?page=${startPage + 10}">[다음]</a>
			</c:if>
		
			<!-- 마지막 페이지로 이동 -->
			<a href="./TipListAction.tip?page=${pageCount}" style="text-decoration:none"> >> </a>
