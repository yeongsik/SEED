...
<% 	}else{  %>	
	<title>Free_Board_List</title>
		<a href="free_board_write.jsp">글쓰기</a> 글갯수 : <%=count %>
...
%>				
				<tr>	
					<td><%=board.getBoard_num() %></td> <!-- 제목 클릭시 글 상세페이지로 이동. -->
					<td>
						<a href="free_board_content.jsp?listClickFlag=Y&board_num=<%=board.getBoard_num()%>&page=<%=currentPage%>">
																	<%=board.getBoard_subject() %></a>					
					</td>
					<td>
						<%=board.getName() %>
					</td>
					<td>
						<%=sd.format(board.getBoard_register()) %>
					</td>
					<td>
						<%=board.getBoard_view() %>
					</td>
...
%>		
			[<%=i %>]	
<% 		}else{  %>
			<a href="free_board_list.jsp?page=<%=i%>">[<%=i %>]</a>
<%		}
	} // for end
	// 다음 블럭으로 이동하는 부분
